...
`storages.backends.s3boto3` is a backwards-compatibility shim as of
django-storages 1.14 — it re-exports the canonical classes, which now live in
`storages.backends.s3`:

| Canonical (1.14+) | Legacy alias (still works) |
|-------------------|----------------------------|
| `storages.backends.s3.S3Storage` | `storages.backends.s3boto3.S3Boto3Storage` |
| `storages.backends.s3.S3StaticStorage` | `storages.backends.s3boto3.S3StaticStorage` |
| `storages.backends.s3.S3ManifestStaticStorage` | `storages.backends.s3boto3.S3ManifestStaticStorage` |

They are the *same objects* (`S3Boto3Storage is S3Storage`), so existing configs
keep working and no migration is urgent. Prefer the canonical path in new code.
